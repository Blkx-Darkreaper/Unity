...
public class EvasiveManeuver : MonoBehaviour {
	...
	public void FixedUpdate () 
    {
        ...
        craft.position = new Vector3(
            Mathf.Clamp(craft.position.x, bounds.xMin, bounds.xMax),
            0f,
            Mathf.Clamp(craft.position.z, bounds.zMin, bounds.zMax)
        );
    ...
    private IEnumerator Evade()
    {
        ...
        while (true)
        {
            ...
            yield return new WaitForSeconds(Random.Range(strafeTime.min, strafeTime.max));
            strafeTarget = 0;
            yield return new WaitForSeconds(Random.Range(strafeWait.min, strafeWait.max));
        }
    }
}
